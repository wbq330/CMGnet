...
def get_dataset(dataset_name, target_folder="./", datasets=DATASETS_CONFIG):
    ...
    if dataset_name == 'Houston13':
        ...
        gt = open_file(folder + 'paviaU_gt_7.mat')['paviaU_gt_7']
        ...
        label_queue = {"tree": ['The trees beside road', 'The trees appear as small circles'],
                       ...
                       "asphalt": ['Trees grew along the asphalt road',
                                   'The asphalt road appear as elongated strip shape'],
                       ...
                       "bitumen": ['Bitumen is a material for building surfaces',
                                   'Bitumen is a widely used waterproof material'],

                       "shadow": ['The shadows next to buildings', 'The shadow appears black'],

                       "meadow": ['The surface of the meadow is green with grass',
                                  'The surface of the meadow is green with grass'],

                       "bare soil": ['No grass on the surface of Bare soil',
                                     'The bare soil appears grayish-black color']}
        ...
        gt = open_file(folder + 'pavia_gt_7.mat')['pavia_gt_7']
        ...
        # Load the image
        img = open_file(folder + 'Shanghai.mat')['Shanghai']

        rgb_bands = [20, 30, 30]

        gt = open_file(folder + 'Shanghai_gt.mat')['Shanghai_gt']

        label_values = ["water", "land", "plant"]
        label_queue = {"water":['The water has a smooth surface','The water appears dark blue or black'],
                       "plant": [
                           "The plants show a vibrant green color with dense foliage",
                           "The plants have distinct spectral signatures with high near-infrared reflectance"
                       ],
                       "land": [
                           "The land surface appears dry and cracked with brownish tones",
                           "The bare land exhibits light beige color with minimal vegetation cover"
                       ],
                       "building": [
                           "The building has a rectangular shape with a flat gray roof",
                           "The building shows sharp geometric edges with concrete or glass surfaces"
                       ]
                       }
        ...
        # Load the image
        img = open_file(folder + 'Hangzhou.mat')['Hangzhou']

        rgb_bands = [20, 30, 30]

        gt = open_file(folder + 'Hangzhou_gt.mat')['Hangzhou_gt']
...
class HyperX(torch.utils.data.Dataset):
    """ Generic class for a hyperspectral scene """   # 用于高光谱场景的通用类
